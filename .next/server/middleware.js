...
/* harmony import */ var next_dist_server_web_adapter__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(384);
        ...
        var mod = __webpack_require__(349)
...
function middleware(request) {
    ...
    if (protectedRoutes.includes(request.nextUrl.pathname) && (!currentUser || Date.now() > JSON.parse(currentUser).expiredAt)) {
        ...
        const response = server.NextResponse.redirect(new URL("/login", request.url));
        response.cookies.delete("currentUser");
        return response;
    }
    if (authRoutes.includes(request.nextUrl.pathname) && currentUser) {
        return server.NextResponse.redirect(new URL("/admin", request.url));
    }
}
